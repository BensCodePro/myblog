...
function resize_image($filename, $max_size = 1000)
{
	...
	if(file_exists($filename))
	{
		...
		function uploadPDFFiles() {
			...
			$targetDir = "uploads/";
			$allowTypes = array('jpg', 'png', 'jpeg', 'gif', 'pdf'); // Add 'pdf' to the allowed types
			...
			if (!empty($fileNames)) {
				...
				if (!empty($insertValuesSQL)) {
					...
					$insert = query("INSERT INTO posts(user_id,category_id,title,content,pdf_content,image,date,slug) VALUES $insertValuesSQL");
					if ($insert) {
